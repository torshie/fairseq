...
from fairseq.models.transformer import (
    Embedding, Linear, PositionalEmbedding, TransformerDecoderLayer,
    TransformerEncoder
)


@register_model('aligned_transformer')
class AlignedTransformerModel(FairseqModel):
    """
    Args:
        encoder (AlignedTransformerEncoder): the encoder
        decoder (AlignedTransformerDecoder): the decoder
    ...
    def build_model(cls, args, task):
        ...
        encoder = AlignedTransformerEncoder(args, src_dict, encoder_embed_tokens)
        decoder = AlignedTransformerDecoder(args, tgt_dict, decoder_embed_tokens)
        return AlignedTransformerModel(encoder, decoder)


class AlignedTransformerEncoder(TransformerEncoder):
    def forward(self, src_tokens, src_lengths):
        ...
        Returns:
            dict:
                ...
                - **embedding_out** (Tensor): the first encoder layer's output
...
class AlignedTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, left_pad=False, final_norm=True):
        ...
        assert input_embed_dim == output_embed_dim, \
            "input/output dimension differs: %d != %d" % (input_embed_dim, output_embed_dim)
        ...
        self.align = MultiheadAttention(
            output_embed_dim, 1, dropout=args.attention_dropout)
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        x, attn = self.align(
            query=x,
            key=encoder_out['embedding_out'],
            value=encoder_out['embedding_out'],
